...
const Favorite = () => {
    ...
    useEffect(() => {
        const fetchFavorites = async () => {
            try {
                const response_favorites = await fetch(`http://13.213.1.218:8080/api/favorite`, { credentials: 'include' });
        ...
        const fetchCameraList = async () => {
            const response_list = await fetch(`http://13.213.1.218:8080/api/list`);
        ...
        const fetchCameras = async () => {
            try {
                const response_cameras = favorites.map(async (favorite) => {
                    const response_camera = await fetch(`http://13.213.1.218:8080/api/main/${favorite.cameraId}`);
    ...
    const handleDelete = async (cameraId) => {
        try {
            ...
            const response = await axios.delete('http://13.213.1.218:8080/api/favorite/delete', {
                data: { cameraId },
    ...
    const handleChangePrice = async (cameraId, currentIdealPrice) => {
        ...
        if (newIdealPrice !== null && newIdealPrice !== "" && !isNaN(newIdealPrice)) {
            try {
                ...
                await axios.post('http://13.213.1.218:8080/api/favorite/add', { cameraId, idealPrice: newIdealPrice }, { withCredentials: true });
